...
class TopicAnalysisTextPreparationService:
    ...
    def prepare(self, dataframe: pd.DataFrame, *, text_column_name: str) -> PreparedTextDataset:
        ...
        for row_index, raw_value in dataframe[text_column_name].items():
            ...
            if self._is_single_word_response(normalized):
                skipped_count += 1
                single_word_count += 1
                continue
        ...
        if single_word_count:
            warnings.append(f"Skipped {single_word_count} single-word response(s) before analysis.")
        if truncated_count:
            warnings.append(
    ...
    @classmethod
    def _is_single_word_response(cls, text: str) -> bool:
        tokens = [token.strip("_'") for token in cls.WORD_PATTERN.findall(str(text or "")) if token.strip("_'")]
        return len(tokens) == 1
