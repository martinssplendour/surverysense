...
from app.features.analysis.topic_label_consolidation_service import TopicLabelConsolidationService
...
class TopicAiLabelService:
    ...
    GENERIC_LABELS = TopicLabelValidator.GENERIC_LABELS
    PLACEHOLDER_TOKENS = TopicLabelValidator.PLACEHOLDER_TOKENS
    LABEL_TOKEN_PATTERN = TopicLabelValidator.LABEL_TOKEN_PATTERN
    ...
    def __init__(self, *, config: TopicAiLabelingConfig) -> None:
        ...
        self.validator = TopicLabelValidator()
        self.consolidation_service = TopicLabelConsolidationService(
            enabled=config.consolidate_similar_labels,
            request_with_retries=self._request_label_consolidation_with_retries,
    ...
    def _consolidate_similar_labels(
        ...
        *,
        ...
        return self.consolidation_service.consolidate(
            labels_by_group_id,
            groups=groups,
            model_key=model_key,
            text_column_name=text_column_name,
    ...
    def _parse_label_consolidation(
        ...
        *,
        ...
        return self.consolidation_service.parse_label_consolidation(
            payload,
    ...
    def _is_valid_ai_label(self, label: str, group: TopicLabelEvidenceGroup) -> bool:
        return self.validator.is_valid_ai_label(label, group)

    @classmethod
    def _is_keyword_stitch_label(cls, tokens: list[str]) -> bool:
        return TopicLabelValidator.is_keyword_stitch_label(tokens)

    @classmethod
    def _has_reasonable_length(cls, label: str, tokens: list[str]) -> bool:
        return TopicLabelValidator.has_reasonable_length(label, tokens)

    @staticmethod
    def _has_excessive_repetition(tokens: list[str]) -> bool:
        return TopicLabelValidator.has_excessive_repetition(tokens)

    def _is_unsupported_generic_label(self, tokens: list[str], group: TopicLabelEvidenceGroup) -> bool:
        return self.validator.is_unsupported_generic_label(tokens, group)

    def _evidence_tokens(self, group: TopicLabelEvidenceGroup) -> set[str]:
        return self.validator.evidence_tokens(group)

    @classmethod
    def _label_tokens(cls, label: str) -> list[str]:
        return TopicLabelValidator.label_tokens(label)

    @classmethod
    def _normalize_for_validation(cls, label: str) -> str:
        return TopicLabelValidator.normalize_for_validation(label)
