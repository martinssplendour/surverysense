...
class MetadataColumnSelectionService:
    ...
    def is_metadata_column(self, column_name: str) -> bool:
        ...
        if self._looks_like_identifier_header(tokens):
            return True
        if self._looks_like_named_identifier_header(tokens):
            return True
    ...
    @classmethod
    def _looks_like_named_identifier_header(cls, tokens: list[str]) -> bool:
        """Return True for short name/email columns that can identify a respondent or organisation."""
        if not tokens or len(tokens) > 4:
            return False
        if any(token in cls.QUESTION_LIKE_TOKENS for token in tokens):
            return False
